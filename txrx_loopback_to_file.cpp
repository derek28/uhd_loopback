...
static const std::string GPIO_DEFAULT_GPIO       = "TXA";
static const size_t GPIO_DEFAULT_NUM_BITS        = 16;
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    // gpio variablese to be set by po
    std::string gpio;
    size_t num_bits;
    std::string ctrl_str;
    std::string ddr_str;
    ...
    desc.add_options()
        ...
        ("tx-rate", po::value<double>(&tx_rate)->default_value(5e6), "rate of transmit outgoing samples")
        ("rx-rate", po::value<double>(&rx_rate)->default_value(5e6), "rate of receive incoming samples")
        ("tx-freq", po::value<double>(&tx_freq)->default_value(0), "transmit RF center frequency in Hz")
        ("rx-freq", po::value<double>(&rx_freq)->default_value(0), "receive RF center frequency in Hz")
        ("ampl", po::value<float>(&ampl)->default_value(float(0.3)), "amplitude of the waveform [0 to 0.7]")
        ("tx-gain", po::value<double>(&tx_gain)->default_value(10), "gain for the transmit RF chain")
    ...
    tx_usrp->set_gpio_attr(gpio, "DDR", ddr, mask);

    // set control register
    tx_usrp->set_gpio_attr(gpio, "CTRL", ctrl, mask);

    // set output values (OUT)
    tx_usrp->set_gpio_attr(gpio, "OUT", out, mask);

    // set ATR registers
    tx_usrp->set_gpio_attr(gpio, "ATR_0X", atr_idle, mask);
    tx_usrp->set_gpio_attr(gpio, "ATR_RX", atr_rx, mask);
    tx_usrp->set_gpio_attr(gpio, "ATR_TX", atr_tx, mask);
    tx_usrp->set_gpio_attr(gpio, "ATR_XX", atr_duplex, mask);
